...
function DashboardGreeting({ fullName, role, lastLogin }: { fullName: string; role: string; lastLogin?: string | null }) {
  ...
  const greeting = hour === null ? `Welcome back, ${firstName}.` : hour >= 5 && hour < 12 ? `The field is still. Good morning, ${firstName}.` : hour < 17 ? `The arms are moving. Afternoon, ${firstName}.` : hour < 21 ? `The light is fading. Evening, ${firstName}.` : `Still building. Late night, ${firstName}.`;
  return <header className="border-b border-mercury pb-6"><p className="font-mono text-[10px] uppercase tracking-[.18em] text-plasma">Dashboard</p><h1 className="mt-3 font-display text-4xl leading-none sm:text-5xl">{greeting}</h1><div className="mt-4 flex flex-wrap items-center gap-x-2 gap-y-1 font-mono text-[9px] uppercase tracking-[.12em] text-ghost"><span className="h-1.5 w-1.5 rounded-full bg-flux shadow-[0_0_8px_rgba(0,255,194,.85)]" /> <span>{fullName}</span><span>·</span><span>{role.replaceAll("_", " ")}</span><span>·</span><span>Whole Body Records</span>{lastLogin ? <><span>·</span><span>Last login {relativeTime(lastLogin)}</span></> : null}</div></header>;
}
...
function LiveNow({ items, generatedAt }: { items: DashboardLiveItem[]; generatedAt: string }) {
  ...
  return <section className="border border-mercury bg-carbon"><header className="flex flex-wrap items-center justify-between gap-3 border-b border-mercury px-5 py-4"><div><p className="flex items-center gap-2 font-mono text-[10px] uppercase tracking-[.16em] text-bone"><span className="h-2 w-2 rounded-full bg-flux shadow-[0_0_10px_rgba(0,255,194,.8)]" />Live now</p><p className="mt-1 text-xs text-ghost">What is happening across the field in this moment.</p></div><p className="font-mono text-[8px] uppercase tracking-[.14em] text-ghost">Updated {refreshedAt} · refreshes each minute</p></header><div className="p-3 sm:p-4">{items.length ? <div className="grid gap-2">{items.map((item) => <Link className="group grid gap-3 border border-steel bg-void/60 p-4 transition hover:border-flux sm:grid-cols-[minmax(0,1fr)_auto] sm:items-center" href={item.href} key={item.id}><div><p className={`font-mono text-[9px] uppercase tracking-[.13em] ${tone[item.tone]}`}>{item.label}</p><p className="mt-2 text-sm text-bone">{item.title}</p><p className="mt-1 text-xs leading-5 text-ghost">{item.detail}</p></div><span className="inline-flex items-center gap-1 font-mono text-[9px] uppercase tracking-[.12em] text-flux">{item.action}<ArrowUpRight className="h-3.5 w-3.5" /></span></Link>)}</div> : <div className="border border-dashed border-steel p-6"><p className="font-mono text-[10px] uppercase tracking-[.13em] text-ghost">● The field is quiet.</p><p className="mt-2 text-sm text-ghost">No active sessions, practices, or urgent leads right now.</p></div>}</div></section>;
}
...
function CountdownClock({ countdown }: { countdown: DashboardCountdown }) {
  ...
  if (!countdown) return <section className="border border-dashed border-steel bg-carbon px-5 py-8"><p className="font-mono text-[10px] uppercase tracking-[.15em] text-ghost">Mission clock</p><h2 className="mt-3 font-display text-3xl leading-none">No shows scheduled. The field awaits.</h2></section>;
  return <Link className="group block overflow-hidden border border-plasma/45 bg-[radial-gradient(circle_at_85%_15%,rgba(176,38,255,.2),transparent_28%),#0a0a0f] p-5 transition hover:border-plasma sm:p-7" href={countdown.href}><div className="grid gap-6 sm:grid-cols-[auto_minmax(0,1fr)] sm:items-center"><p className="font-display text-7xl leading-none text-plasma drop-shadow-[0_0_24px_rgba(176,38,255,.48)] sm:text-8xl">{countdown.days}</p><div><p className="font-mono text-[10px] uppercase tracking-[.18em] text-plasma">Days until {countdown.title}</p><h2 className="mt-3 font-display text-3xl leading-none text-bone sm:text-4xl">{dateLabel(countdown.date)} · {countdown.artist}</h2><div className="mt-5 flex flex-wrap gap-x-2 gap-y-1 font-mono text-[9px] uppercase tracking-[.1em]">{phases.map((phase, index) => <span className={index < countdown.phasePosition ? "text-flux" : index === countdown.phasePosition ? "text-plasma" : "text-ghost"} key={phase}>{index < countdown.phasePosition ? "✓" : index === countdown.phasePosition ? "●" : "○"} {phase}{index < phases.length - 1 ? " →" : ""}</span>)}</div><p className="mt-3 text-xs text-ghost">Current phase: {countdown.phase}</p></div></div><span className="mt-5 inline-flex items-center gap-1 font-mono text-[9px] uppercase tracking-[.12em] text-flux opacity-0 transition group-hover:opacity-100">Open show record <ArrowUpRight className="h-3.5 w-3.5" /></span></Link>;
}
...
function MorningCards({ cards }: { cards: DashboardCard[] }) {
  ...
  return <section><SectionTitle eyebrow="Daily briefing" title="Morning cards" detail="The points that need a decision, a reply, or a glance." /><div className="mt-4 grid gap-px border border-mercury bg-mercury sm:grid-cols-2 xl:grid-cols-4">{cards.map((card) => { const Icon = iconByCard[card.icon]; return <Link className="group bg-carbon p-4 transition hover:bg-steel hover:shadow-[inset_0_0_0_1px_rgba(176,38,255,.8)]" href={card.href} key={card.id}><div className="flex items-center justify-between"><p className="font-mono text-[9px] uppercase tracking-[.13em] text-ghost">{card.label}</p><Icon className={`h-4 w-4 ${tone[card.tone]}`} /></div><p className={`mt-5 font-display text-4xl leading-none ${tone[card.tone]}`}>{card.value}</p><p className="mt-2 min-h-8 text-xs leading-4 text-ghost">{card.detail}</p><div className="mt-5 flex items-end justify-between gap-3"><p className={`font-mono text-[8px] uppercase tracking-[.1em] ${tone[card.tone]}`}>● {card.status}</p><ArrowUpRight className="h-3.5 w-3.5 text-flux opacity-0 transition group-hover:opacity-100" /></div></Link>; })}</div></section>;
}
...
function ArmStatus({ arms }: { arms: DashboardArm[] }) {
  ...
  return <section><SectionTitle eyebrow="The five arms" title="The center holds." detail="Every arm has work in motion, and every signal returns to Ø." /><div className="mt-4 grid gap-3 md:grid-cols-2 xl:grid-cols-5">{arms.map((arm) => <Link className={`group flex min-h-56 flex-col border border-mercury border-l-4 ${color[arm.color]} bg-carbon p-4 transition hover:bg-steel`} href={arm.href} key={arm.id}><div><p className="font-mono text-[10px] uppercase tracking-[.15em] text-bone">{arm.label}</p><p className="mt-2 text-xs text-ghost">● {arm.description}</p></div><div className="mt-7"><p className="font-mono text-[9px] uppercase tracking-[.1em] text-flux">Active</p><p className="mt-2 text-sm leading-5 text-bone">{arm.activity}</p><p className="mt-3 text-xs text-ghost">{arm.metric}</p></div><div className="mt-auto pt-6">{arm.alert ? <p className="font-mono text-[8px] uppercase leading-4 tracking-[.09em] text-halo">Alert · {arm.alert}</p> : <p className="font-mono text-[8px] uppercase tracking-[.09em] text-ghost">No immediate alert</p>}<p className="mt-4 inline-flex items-center gap-1 font-mono text-[9px] uppercase tracking-[.12em] text-flux">Enter arm <ArrowUpRight className="h-3.5 w-3.5" /></p></div></Link>)}</div></section>;
}

const markets = [
  { className: "swap-map__market--joshua", name: "Joshua Tree", role: "Home base", x: 500, y: 164 },
  { className: "swap-map__market--la", name: "Los Angeles", role: "Major market", x: 287, y: 324 },
  { className: "swap-map__market--sd", name: "San Diego", role: "Growing scene", x: 357, y: 474 },
];

export function SwapNetwork() {
  return (
    <figure className="swap-network">
      <svg aria-labelledby="exchange-map-title exchange-map-description" className="swap-map" role="img" viewBox="0 0 680 580" xmlns="http://www.w3.org/2000/svg">
        ...
        <path className="swap-map__roads" d="M467 145 287 324 357 474M500 164 357 474M287 324 500 164" />
        <rect className="swap-map__grid" height="580" width="680" />

        {markets.map((market) => <g className={`swap-map__market ${market.className}`} key={market.name} transform={`translate(${market.x} ${market.y})`}>
          <circle className="swap-map__halo" r="20" />
          <circle className="swap-map__point" r="7" />
          <circle className="swap-map__core" r="2.4" />
        </g>)}

        <g className="swap-map__label swap-map__label--joshua" transform="translate(520 151)"><text>Joshua Tree</text><tspan x="0" y="17">Home base</tspan></g>
        <g className="swap-map__label swap-map__label--la" transform="translate(102 312)"><text>Los Angeles</text><tspan x="0" y="17">Major market</tspan></g>
        <g className="swap-map__label swap-map__label--sd" transform="translate(378 504)"><text>San Diego</text><tspan x="0" y="17">Growing scene</tspan></g>
      ...
      <figcaption className="swap-network-status"><span className="live-dot" />Pilot exchange lanes · Southern California</figcaption>
    </figure>
  );
}
